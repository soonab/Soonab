...
export async function POST(req: NextRequest) {
  ...
  if (body.length > POST_MAX) {
    return NextResponse.json(
      { ok: false, error: `Too long (max ${POST_MAX})` },
      { status: 400 }
    )
  }
  ...
  // ----------------------------
  // Ensure public profile + quota check
  // ----------------------------
  await ensureSessionProfile(sid)
  const gate = await canCreatePost(sid)
  if (!gate.ok) {
    return NextResponse.json(
  ...
  // IMPORTANT: secure cookie only in production, or it won't set on localhost
  if (setCookie) {

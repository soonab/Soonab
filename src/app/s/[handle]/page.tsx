...
export default async function ProfilePage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  if (profile) {
    ...
    // POSTS: respect viewer visibility AND moderation state
    const posts = await prisma.post.findMany({
      ...
      take: 50,
      include: {
        // REPLIES: public-only (for now) AND moderation state
        replies: {
  ...
  // Legacy: pseudonymous session handle → PUBLIC-only, and only ACTIVE content
  ...
  const posts = await prisma.post.findMany({
    where: { sessionId: session.sessionId, visibility: 'PUBLIC', state: 'ACTIVE' },
    orderBy: { createdAt: 'desc' },
    take: 50,
    include: {
      replies: {

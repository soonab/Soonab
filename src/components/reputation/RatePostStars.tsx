...
type Props = {
  postId: string;
  initialLocked?: boolean; // true if this viewer already rated this post
  size?: number;
  disabled?: boolean;      // e.g., own post
};

export default function RatePostStars({ postId, initialLocked = false, size = 14, disabled = false }: Props) {
  ...
  // Minimal visual: only stars + tiny label. No "4.0/5" or percent text here.
  return (
        ...
        {stars.map((s) => {
          ...
          return (
            <button
              ...
              style={{ lineHeight: 0 }}
              title={locked ? 'Rating locked' : 'Rate this post'}
            >

...
export async function canCreateReply(sessionId: string, postId: string) {
  ...
  if (usedDay >= q.repliesPerDay) return { ok: false as const, error: 'Daily reply limit reached' as const }
  if (usedThread >= q.perThreadDaily) return { ok: false as const, error: 'Thread reply limit reached' as const }
  return { ok: true as const, quota: q }
}

...
export function quotasForScore(bm: number) {
  if (bm >= 4.5) return { postsPerDay: 4, repliesPerDay: 24, perThreadDaily: 12, tier: 'A+' as const };
  if (bm >= 4.0) return { postsPerDay: 3, repliesPerDay: 18, perThreadDaily: 9,  tier: 'A'  as const };
  if (bm >= 3.0) return { postsPerDay: 2, repliesPerDay: 12, perThreadDaily: 6,  tier: 'B'  as const };
  return             { postsPerDay: 1, repliesPerDay: 6,  perThreadDaily: 3,  tier: 'C'  as const };
}
...
export async function recomputeScore(targetSessionId: string) {
  ...
  const raterIds = Array.from(
    new Set((ratings.map(r => r.raterSessionId).filter(Boolean) as string[]))
  );
  ...
  const scoreMap = new Map<string, number>(
    raterScores.map(s => [s.sessionId, s.bayesianMean ?? PRIOR_MEAN])
  );
  ...
  for (const r of ratings) {
    const raterId = r.raterSessionId ?? null;
    const bayes = raterId ? scoreMap.get(raterId) ?? PRIOR_MEAN : PRIOR_MEAN;
    const w = weightFromRaterScore(Number(bayes)) * recencyFactor(r.updatedAt);
    wSum += r.value * w;
    wCnt += w;
  }
...
export async function recomputeScoreForProfile(targetProfileId: string) {
  ...
  const raterProfileIds = Array.from(
    new Set((ratings.map(r => r.raterProfileId).filter(Boolean) as string[]))
  );
  const raterSessionIds = Array.from(
    new Set((ratings.map(r => r.raterSessionId).filter(Boolean) as string[]))
  );
  ...
  const pMap = new Map<string, number>(
    byProfile.map(s => [s.profileId!, s.bayesianMean ?? PRIOR_MEAN])
  );
  const sMap = new Map<string, number>(
    bySession.map(s => [s.sessionId, s.bayesianMean ?? PRIOR_MEAN])
  );
  ...
  for (const r of ratings) {
    const bayes =
      (r.raterProfileId ? pMap.get(r.raterProfileId) : undefined) ??
      (r.raterSessionId ? sMap.get(r.raterSessionId) : undefined) ??
      PRIOR_MEAN;

    // TS fix: coerce to number explicitly
    const w = weightFromRaterScore(Number(bayes)) * recencyFactor(r.updatedAt);
    wSum += r.value * w;
    wCnt += w;
  }
  ...
  const existingProfileRow = await prisma.reputationScore.findFirst({
...
export async function maybeFlagBrigade(targetSessionId: string) {
  ...
  const uniqueCount = new Set(recent.map(r => r.raterSessionId).filter(Boolean)).size;
